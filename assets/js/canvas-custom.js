	...
	canvas.on("selection:created", function(e) {
	  ...
	  if(activeObj.type === "group") {
		...
		e.target.forEachObject(function(obj) {
			...
			$('.ed_group_colorPicker').colorpicker().on('change', function(e) {
				...
				var path = fabric.loadSVGFromString(data,function(objects, options) {
					for(var i=0;i<objects.length;i++){
						...
						html = '<div class="pg-colorPicker-container" data-index="'+i+'">';
							html += '<input type="text" class="pg_colorPicker input-group-addon" value="'+color+'" /><span class="color_viewer" style="background-color: '+color+';"></span>';
						html += '</div>';
